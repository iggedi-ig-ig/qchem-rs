...
use super::{BasisFunction, ContractedGaussian};
...
impl AtomicBasis {
    ...
    pub(crate) fn basis_for(&self, atom: &Atom) -> Vec<BasisFunction> {
        self.shells
            .iter()
            .flat_map(|shell| &shell.basis_functions)
            .map(|contracted_gaussian| BasisFunction {
